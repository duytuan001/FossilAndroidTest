package com.duytuan.screeningtest.question2.widget.timepicker
...
import com.duytuan.screeningtest.question2.widget.timepicker.Utils.Companion.angleBetweenVectors
import com.duytuan.screeningtest.question2.widget.timepicker.Utils.Companion.angleToMins
import com.duytuan.screeningtest.question2.widget.timepicker.Utils.Companion.snapMinutes
import com.duytuan.screeningtest.question2.widget.timepicker.Utils.Companion.to_0_720
...
//    fun getWakeTime() = computeWakeTime()
    ...
    fun setWakeTime(wakeTime: LocalTime) {
        wakeAngle = Utils.minutesToAngle(wakeTime.hour * 60 + wakeTime.minute)
        invalidate()
        notifyChanges()
    }
...
//        get() = progressPaint.strokeWidth
    ...
    private fun init(@NonNull context: Context, @Nullable attrs: AttributeSet?) {
        ...
        if (attrs != null) {
            ...
            progressBackgroundColor =
                a.getColor(
                    R.styleable.SleepTimePicker_progressBackgroundColor,
                    progressBackgroundColor
                )
            divisionColor = a.getColor(R.styleable.SleepTimePicker_divisionColor, divisionColor)
            progressStrokeWidth =
                a.getDimensionPixelSize(
                    R.styleable.SleepTimePicker_progressStrokeWidth,
                    progressStrokeWidth
                )
            progressBottomShadowSize =
                a.getDimensionPixelSize(R.styleable.SleepTimePicker_strokeBottomShadowRadius, 0)
            progressTopShadowSize =
                a.getDimensionPixelSize(R.styleable.SleepTimePicker_strokeTopShadowRadius, 0)
            progressBgStrokeWidth =
                a.getDimensionPixelSize(
                    R.styleable.SleepTimePicker_progressBgStrokeWidth,
                    progressStrokeWidth
                )
            strokeBottomShadowColor =
                a.getColor(R.styleable.SleepTimePicker_strokeBottomShadowColor, progressColor)
            strokeTopShadowColor =
                a.getColor(R.styleable.SleepTimePicker_strokeTopShadowColor, progressColor)
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        val smallestSide = min(measuredWidth, measuredHeight)
//        Timber.d("onMeasure $smallestSide")
    ...
    override fun onTouchEvent(ev: MotionEvent): Boolean {
        //To make it touchable when inside of a scrollview // My custom
        parent.requestDisallowInterceptTouchEvent(draggingSleep || draggingWake)
